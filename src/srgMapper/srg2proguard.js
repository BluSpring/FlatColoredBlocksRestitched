...
module.exports = class SRG2ProGuard {
    ...
    fieldRegex = /field_\d+_/;
    methodRegex = /func_\d+_/;
    //classRegex = /(\w+(\/)?)+\/C_\d+_/;
    ...
    async map() {
        ...
        for (const line of split) {
            ...
            //console.log(parts)
            if (!!parts[1] && !parts[1].includes('(') && parts[1].includes('/')/* && this.classRegex.test(parts[1])*/) {
                className = parts[0];
                //console.log(`Mapping class ${className}...`);
            } else if (!!parts[1] && parts[1].startsWith('field')) {
            ...
            } else if (!!parts[2] && parts[2].startsWith('func')) {
